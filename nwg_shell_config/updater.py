...
def main():
    ...
    if current_shell_version > shell_data["installed-version"]:
        for version in __need_update__:
            if (
                is_newer(version, shell_data["installed-version"])
                and version not in shell_data["updates"]
            ):
                version_descriptions.append(
                    load_text_file(os.path.join(updates_dir, version))
                )
    ...
    lbl = Gtk.Label()
    lbl.set_markup(
        'nwg-shell-updater  <a href="https://nwg-piotr.github.io/nwg-shell/updates">Updates page</a>'
    )
