...
def lockscreen_tab(settings, voc):
    ...
    entry_keywords.set_tooltip_text(voc["tags-tooltip"])
    entry_keywords.set_placeholder_text("nature,landscape")
    entry_keywords.set_text(",".join(settings["wallhaven-tags"]))
    entry_keywords.connect("changed", set_keywords_from_entry, settings)
    grid.attach(entry_keywords, 3, 8, 1, 1)
    ...
    lbl = Gtk.Label()
    lbl.set_markup("{}:".format(voc["minimum-resolution"]))
    lbl.set_property("halign", Gtk.Align.END)
    lbl.set_property("margin-top", 6)
    grid.attach(lbl, 2, 10, 1, 1)

    entry_atleast = Gtk.Entry()
...
def dock_tab(preset, preset_name, outputs, voc):
    ...
    cb_nolauncher = Gtk.CheckButton.new_with_label(voc["dock-nolauncher"])
    cb_nolauncher.set_active(preset["dock-nolauncher"])
    cb_nolauncher.connect("toggled", set_from_checkbutton, preset, "dock-nolauncher")
    grid.attach(cb_nolauncher, 1, 11, 1, 1)
    ...
    if os.getenv("HYPRLAND_INSTANCE_SIGNATURE"):
        ...
        grid.attach(cb_blur_bg, 1, 12, 1, 1)
